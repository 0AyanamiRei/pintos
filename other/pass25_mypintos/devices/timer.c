...
timer_interrupt (struct intr_frame *args UNUSED) {
  ...
  if(thread_mlfqs) {
    ...
    increase_recent_cpu(t);    /** 每1个tick自增`recent_cpu` */
    if(timer_ticks() % TIMER_FREQ == 0){
      update_load_avg();   /** 每秒更新load_avg */
      thread_foreach(update_recent_cpu, NULL);  /** 每秒为所有线程重新计算一遍`recent_cpu` */
    }

    if(timer_ticks() % 4 == 0) {
      thread_foreach(update_priority, NULL); /** 每4个ticks重新计算一遍`priority` */
    }
  }
}
